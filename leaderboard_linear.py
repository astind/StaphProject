...
def get_data(filename):
    ...
    for x in spectrum:
        val = float(x)
        spec_int.append(int(math.trunc(val)))
    return spec_int, sequence
...
def score2(peptide, exp_spec):
    if peptide is None:
        return 0
    pep_spec = linear_spectrum(peptide)
    temp_exp = exp_spec.copy()
    score = 0
    for pep in pep_spec:
        least_diff = math.inf
        least_val = None
...
def pep_to_mass(sequence):
    mass_list = []
    for seq in sequence:
        mass_list.append(mass_table[seq])
    return mass_list


def threshold(num1, num2):
    thresh = 100
    if num1 in range(num2 - thresh, num2 + thresh):
        return True
    return False


def find_peptide(sequence, spectrum):
    leader = None
    leader_score = 0
    pep_length = 22
    for i in range(len(sequence) - (pep_length - 1)):
        sample_seq = sequence[i:i+pep_length]
        sample_mass = pep_to_mass(sample_seq)
        if threshold(mass(sample_mass), parent_mass(spectrum)):
            sample_score = score2(sample_mass, spectrum)
            if sample_score > leader_score:
                leader = sample_mass
                leader_score = sample_score
    return leader, leader_score


if __name__ == '__main__':
    spectrum, sequence = get_data(sys.argv[1])
    ans, ans_score = find_peptide(sequence, spectrum)
    print(ans)
    print(linear_spectrum(ans))
    print(ans_score)
